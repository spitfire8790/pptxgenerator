...
const getHeritageData = async (geometry) => {
    ...
    try {
        ...
        if (response.features && response.features.length > 0) {
            // Return all features' attributes
            return response.features.map(f => f.attributes);
        }
        return [];
    } catch (error) {
        console.error('Error querying heritage data:', error);
        return [];
    }
};
...
export async function addPlanningSlide2(pptx, properties) {
  ...
  if (properties.developableArea && properties.developableArea.length > 0) {
    const geometry = {
      rings: properties.developableArea.map(area => area.geometry.coordinates[0])
    };
   ...
   try {
    ...
    if (heritageData && heritageData.length > 0 && (heritageScore === 1 || heritageScore === 2)) {
      // Get the most significant heritage item (with lowest score)
      const significantHeritage = heritageData.find(item => {
        const sig = item.SIG?.toLowerCase() || '';
        return heritageScore === 1 ? 
          (sig.includes('state') || sig.includes('national')) : 
          sig.includes('local');
      });
            
      if (significantHeritage) {
        if (significantHeritage.LAY_CLASS) {
          heritageText += `\n\nClass: ${significantHeritage.LAY_CLASS}`;
        }
        if (significantHeritage.H_NAME) {
          heritageText += `\nName: ${significantHeritage.H_NAME}`;
        }
      }
    }
  } catch (error) {
