...
const DevelopableAreaSelector = ({ 
  ...
  // Handle auto-generated developable area
  const handleDevelopableAreaGenerated = (developableArea) => {
    if (onLayerSelect && developableArea) {
      onLayerSelect(developableArea);
      
      // Refresh the layer list after a short delay to include the newly created layer
      setTimeout(() => {
        fetchLayers();
      }, 1000);
    }
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow h-full">
      ...
      {drawingLayers.length === 0 ? (
        <p className="text-gray-500">No layers available. Create one using Giraffe's drawing tools or use the auto-generate button.</p>
      ) : (
          ...
          {drawingLayers.map((layer) => (
            ...
            >
              ...
              <span className="flex items-center">
                ...
                {layer.layerId}
                {layer.properties?.autoGenerated && (
                  <span className="ml-2 px-1.5 py-0.5 text-xs rounded-full bg-purple-100 text-purple-800">Auto</span>
                )}
              </span>
