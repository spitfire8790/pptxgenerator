...
// Screenshot types
const SCREENSHOT_TYPE_HOB = 'hob';
const SCREENSHOT_TYPE_AERIAL = 'aerial';

// Layer configurations
const LAYER_CONFIG_HOB = {
...
export async function captureHOBMap(feature, developableArea = null, showDevelopableArea = true, useDevelopableAreaForBounds = false, showLabels = true, showDevelopableAreaLabels = true) {
    if (!feature) return null;
    
    try {
      const config = LAYER_CONFIG_HOB;
      ...
      try {
        // 2. HOB layer - Use arcgisService for consistent CRS handling
        console.log('Loading HOB layer...');
        const hobLayer = await getArcGISImage(config, centerX, centerY, size);
        console.log('HOB layer loaded');
        drawImage(ctx, hobLayer, canvas.width, canvas.height, 0.7);
      } catch (error) {
        console.warn('Failed to load HOB layer:', error);
      }
      ...
      console.error('Failed to capture HOB map:', error);
      return null;
    }
  }